...
# URL
$releases = 'https://www.codecguide.com/download_k-lite_codec_pack_standard.htm'
...
function global:au_SearchReplace {
    @{
        ...
        ".\k-litecodecpack-standard.nuspec" = @{
...
function global:au_GetLatest {
    ...
    if ($download_page.Content -match 'Version\s+([0-9.]+)\s+Standard') {
        $version = $matches[1]
    } else {
    ...
    if ($majorGeeksLink) {
        ...
        $url = ($mgPage.Links | Where-Object { $_.href -match 'files3\.codecguide\.com/.*Standard\.exe' } | Select-Object -First 1).href
    } else {
        $url = ($download_page.Links | Where-Object { $_.href -match 'files3\.codecguide\.com/.*Standard\.exe' -and $_.href -notmatch 'old' } | Select-Object -First 1).href
    }
    if (-not $url) {
    ...
    # Извлекаем MD5 из текста (после "MD5: ")
